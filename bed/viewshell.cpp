...
BviewShell::~BviewShell()
{
	...
	// flag that we're self-stopping the thread
	m_destroying = true;
	if(m_running)
	{
		Stop();
	}
	// wait for stop a bit, to avoid accessing deleted vars
	//
	for(wait = 0; (m_pid != 0) && (wait < 100); wait++)
		Bthread::Sleep(10);
}
...
ERRCODE BviewShell::ShellThread()
{
	...
	// shell is over, wait for reader to catch up (but not forever)
	// unless shell was self-stopped because destroying object
	//
	endwait = 0;
