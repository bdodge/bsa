...
ERRCODE BgrepInfo::InsertGrepLine(LPCTSTR grepLine, int maxtimems)
{
	...
	//
	nNeeded = _tcslen(grepLine) * 3; // need extra to encode
...
ERRCODE BgrepInfo::ShellThread()
{
	...
	if(GetInsertHeader())
	{
		...
		_sntprintf(grepLine, 256, _PFT("Search for: %s in %s"), m_grepPat, m_grepDir);
		...
		if(m_ftypeList)
		{
			...
			for(pFilePat = m_ftypeList; pFilePat && m_running; pFilePat = pFilePat->GetNext(pFilePat))
			{
				...
				if(filePart)
				{
					...
					//_tprintf(_PFT("compare=%s pat %s= res=%d\n"),
	...
	if(GetInsertHeader())
	{
		...
		_sntprintf(grepLine, 256, _PFT("Search complete: %d occurances in %d files"), m_occurenceCount, m_fileCount);
		InsertGrepLine(grepLine, 5000);
	}
		...
		while(m_running && ((ec = pBuf->Locate(line, col, m_grepPat, m_nPat, m_casesensi, false)) == errOK))
		{
			...
			else
			{
				...
				if(ec == errOK)
				{
					...
					tlen = _sntprintf(tempText, 512, _PFT("%s (%d) "), name, line);
					room = 510 - tlen;
					if(nText > 128)
						nText = 128;
					if(nText > room)
