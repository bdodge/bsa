...
void BviewTerminal::Activate()
{
	...
	if(! m_running)
	{
		...
		if((pp = GetEditor()->GetPersist()) != NULL)
		{
			...
			ppl = _sntprintf(pparm, 128, _PFT("%s/Emulation/Settings/"), GetTypeName());
			px = pparm + ppl;
			_tcscpy(px, _T("Echo"));
...
COLORREF BviewTerminal::VTrgbColor(char color, int isbold)
{
	...
	case 0: // "tango"
		...
		else
		{
			...
			case 2: cr = RGB(138, 226, 52); break;
...
void BviewTerminal::InsertToken(bool insert)
{
	...
	#ifdef LOG_VT
	m_log->Log(logDebug, 5, _PFT("insert %d,%d =%s=\n"), m_vtrow, m_vtcol, m_intok);
	#endif
		...
		if(! insert)
		{
			...
			if(nText > 0)
			{
				...
				if(nDelete)
				{
					...
					m_log->Log(logDebug, 5, _PFT("removing %d bytes at buffer %d for insert\n"), nDelete, curcol);
#endif
			...
			else
			{
				...
				// make sure there's a new line at the end of each line in buffer
				//
				m_buffer->Insert(m_vtrow, col, _T("\n"), 1);
...
#ifdef LOG_VT
		m_log->Log(logDebug, 5, _PFT("insert tok =%s= at buffer %d,%d\n"), m_intok, m_vtrow, curcol);
#endif
		m_buffer->Insert(m_vtrow, curcol, token, tokcnt);
#ifdef LOG_VT
		ec = m_buffer->GetLineText(m_vtrow, lpText, nText);
		VTdumpToken(m_log, lpText, nText);
		m_log->Log(logDebug, 5, _PFT("inserted tok\n"));
#endif
		m_vtcol = (curcol + 2) / 3;
		m_totalcnt+= tokcnt;
...
void BviewTerminal::UpdateVtPos(int line, int col)
{
...
#ifdef LOG_VT
	m_log->Log(logDebug, 5, _T("pre updatevtpos=%d,%d fl=%d= goto %d,%d\n"),
			m_vtrow, m_vtcol, m_frameline, line, col);
#endif
	...
	do
	{
		ml = m_buffer->GetLineCount(ml);
		if(ml <= line)
		{
			m_buffer->Insert(ml, ll, _T("\n"), 1);
...
{	
	...
	if(m_cnt > 0)
	{
		...
		//
		for(n = 0, nonwhite = false; n < nText; n++)
		{
			c = m_szInp[n];
			if(c == _T(' ') || c == _T('\t'))
			{
			...
			if(m_vtState == vtpsDONE)
			{
...
#ifdef LOG_VT
						m_intok[m_tokcnt] = 0;
						m_log->Log(logDebug, 5, _PFT("vtnone = nw=%d nwd=%d  %d,%d =%s=\n"),
...
#ifdef LOG_VT
					m_log->Log(logDebug, 4, _T("newline\n"));
#endif
					if(m_tokcnt > 0)
					{
						InsertToken(false);
...
#endif								
									m_buffer->Delete(i, xcol, nLineText - xcol - 1);
								}
								UpdateVtPos(m_vtrow - m_frameline + 1, m_vtcols);
								UpdateVtPos(m_vtrow - m_frameline + 2, 1);
								xcol = 1;
							}
							UpdateVtPos(m_vtrow - m_frameline + 1, m_vtcols);
							m_vtrow = srow;
				...
				case vtactVertPad:
					...
					InsertToken(true);
					nonwhite = false;
					UpdateVtPos(iline - m_frameline + 1 /*+ m_action.f_parms[0]*/, icol); 
					uv = vtrcDISPLAY | vtrcLINE;
					break;
				...
				case vtactScroll:
#ifdef LOG_VT
					m_log->Log(logDebug, 4, _T("scroll: %d\n"), m_action.f_parms[0]);
#endif
					break;
...
static BOOL CALLBACK VTtermproc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		if(pp)
		{
			ppl = _sntprintf(pparm, 128, _PFT("%s/Emulation/Settings/"), pem->f_typename);
			px = pparm + ppl;
