...
HFONT _w_newFont(LPCTSTR pFace, LPTEXTMETRIC ptm, BOOL antialias)
{
	...
	// usually call create font with a point size scaled backwards
	...
	if (! strcmp(szFace, "helvetica") || ! strcmp(szFace, "arial"))
	{
		// these are sometimes not on Linux systems, so go through a
		// compatible list
		//
		helvsubex = 1;
	}
...
#endif
	#ifdef DBG_FONTS
	printf("scaled %lld to %d pts, face in=%s out=%s\n",
	...
	// not found, so make it / get it from OS
	//
#ifdef X11
	if(! antialias)
	{
		...
		while(! found)
		{
			...
			case 2:
				// for helvetica/arial, try another face
				if (helvsubex > 0)
				{
					if (HelveticaSubstitutions[helvsubex - 1] != NULL)
					{
						strcpy(szFace, HelveticaSubstitutions[helvsubex - 1]);
						helvsubex++;
						stage = 0;
						break;
					}
	...
	else
	{
		...
		if(xftfont)
		{
			...
			font->tm.tmHeight	= font->tm.tmAscent + font->tm.tmDescent + X_TRA_PADDING;
	...
	if(_tcsstr(pFace, _T("bitstream vera sans")))
	{
		pFace = _T("Courier");
		size += 2;
	}
	if(_tcsstr(pFace, _T("lucida")))
	{
		pFace = _T("Lucida");
	}
	_tcscpy(font->face, pFace);
	...
	font->font = _w_createFont(pFace, &font->tm, &font->tm.tmAscent, &font->tm.tmDescent);
								...
								)
{
	...
	if(nText > 0)
	{
		...
		_w_textExtents(zGC, zFont, text, nText, &w, &h);
		size->cx = w;
		size->cy = h;
	}
	else
	{
		size->cx = 0;
		size->cy = zFont->tm.tmAscent + zFont->tm.tmDescent + X_TRA_PADDING;
	}
	return TRUE;
}
	...
	*/
	...
	if (fs)
	{
		for (i=0; fs && i < fs->nfont; i++)
		{
			...
			if (FcPatternGetString(font, FC_FAMILY, 0, &face) == FcResultMatch)
			{
...
#endif
#ifdef COCOA
	didone = _w_enumFonts(lpFontFunc, lParam);
#endif
	if(! didone)
	{
